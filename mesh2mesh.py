...
class Circle:
  ...
  def add_copper_to_each_drw_from_closest_ir(s: Self):
    ...
    for drw_node in drw_nodes:
      ...
      s.G.add_edge(drw_node, selected, length=s.arcdist(selected, drw_node), type='copper_1')
  ...
  def compute_splitting_cost(s: Self):
    splitters = [n for n in s.ir_nodes if 'M' not in n]
    return len(splitters) * 100
  ...
  def add_fiber_from_exchange_to_first_ring(s: Self):
    ...
    for node in nodes:
      if node.startswith('I_R'):
        s.G.add_edge(s.exchange, node, length=s.dist(s.exchange, node), type='fiber_out')
        s.G.add_edge(node, s.exchange, length=s.dist(s.exchange, node), type='fiber_in')
  ...
  def add_fiber_radially(s: Self):
    for ring in range(1, 4):
      ...
      for node1, node2 in zip(ring_ir, next_ir):
        s.G.add_edge(node1, node2, length=s.dist(node1, node2), type='fiber_out')
        s.G.add_edge(node2, node1, length=s.dist(node1, node2), type='fiber_in')

  
  def connect_ir_to_closest_ir(s: Self, ring: int):
    ir = s.get_ring_org_ir_nodes(ring)
    for node1 in ir:
      dists = {}
      for node2 in ir:
        if node1 == node2: continue
        dists[node2] = s.dist(node1, node2)
      
      iter_dists = iter(sort_dict_by_values(dists))
      selected = next(iter_dists)
      s.G.add_edge(node1, selected, length=dists[selected], type='fiber_side')
  ...
  legend_elements = property(lambda s: [
      Line2D([0], [0], color='orange', lw=2, label='P2P connections radially forward'),
      Line2D([0], [0], color='violet', lw=2, label='P2P connections radially backward'),
      Line2D([0], [0], color='green', lw=2, label='P2P connections same ring'),
      Line2D([0], [0], color='brown', lw=2, label='P2P Node to House and House to Node'),
      Patch(facecolor='Black', edgecolor='black', label=s.exchange),
      Patch(facecolor='red', edgecolor='black', label='Driveway (DRW)'),
      Patch(facecolor='orange', edgecolor='black', label='P2P Nodes'),
  ...
  def plot(s: Self):
    ...
    s.plot_edges(ax, 'fiber_out', 'orange', 'arc3,rad=0.3', width=3)
    s.plot_edges(ax, 'fiber_in', 'violet', 'arc3,rad=0.3', width=3)
    s.plot_edges(ax, 'fiber_side', 'green', 'arc3,rad=0.3', width=3)
    ...
    # ax.set_title('P2P Node to House and House to NOde', fontsize=20)
    fig.text(0.5, 0.02, 'Fig 3.1', ha='right', fontsize=30, color='gray')
    fig.tight_layout()
...
if __name__ == '__main__':
  ...
  c.add_fiber_from_exchange_to_first_ring()
  c.add_driveway_nodes()
  c.add_fiber_radially()
  c.connect_ir_to_closest_ir(2)
  c.connect_ir_to_closest_ir(3)
  c.connect_ir_to_closest_ir(4)
  ...
  # c.add_fiber_radially()
  # c.connect_new_ir_to_closest_ir(2)
  # c.connect_new_ir_to_closest_ir(3)
  # c.connect_new_ir_to_closest_ir(4)
  # c.add_copper_to_each_drw_from_closest_ir()
  c.add_copper_to_each_drw_from_closest_ir()
  print(len(c.ir_nodes))
  c.plot()
  ...
  #   print('splitting cost', c.compute_splitting_cost())
  #   print(
  #     'total cost: ',
  #     sum(
  #       [
